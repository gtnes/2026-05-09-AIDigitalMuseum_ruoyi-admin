...
const [AimuseumModal, modalApi] = useVbenModal({
  connectedComponent: aimuseumModal,
});
...
async function handleEdit(row: Required<AimuseumForm>) {
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: Required<AimuseumForm>) => row.id);
  Modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await aimuseumRemove(ids);
      await tableApi.query();
    },
  });
}
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="AI博物馆列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['system:aimuseum:export']"
          ...
          <a-button
            type="primary"
            v-access:code="['system:aimuseum:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['system:aimuseum:edit']"
    ...
    <AimuseumModal @reload="tableApi.query()" />
  </Page>
</template>
